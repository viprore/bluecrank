@if($status == '입금전')
    <form method="POST" action="{{ route('admin.orders.update', ['order' => $target, 'slug' => 'deposit']) }}"
        ...
        {{ csrf_field() }}
        <button type="submit" class="btn btn-primary">입금완료</button>
    </form>
@endif
...
@if($status == '입금완료')
    <form method="POST" action="{{ route('admin.orders.update', ['order' => $target, 'slug' => 'checked']) }}"
          class="form-horizontal">
        {{ csrf_field() }}
        <button type="submit" class="btn btn-primary">배송준비</button>
    </form>
@endif
...
@if($status == '배송준비')
    <form method="POST" action="{{ route('admin.orders.update', ['order' => $target, 'slug' => 'prepare']) }}"
        ...
        <button type="submit" class="btn btn-primary btn__shipcode">배송중</button>
