    ...
    <div class="container">
        ...
        <form class="form-inline py-3 align-right" role="form" method="GET" action="{{ route('orders.index') }}">
            ...
            <div class="form-group btn-group py-3" role="group" aria-label="term">
                <button type="button"
                        class="btn-pick btn btn-sm {{ date("Y-m-d",strtotime ("+1 days", strtotime(request()->input('start_date')))) == date('Y-m-d') ? 'btn-default':'btn-secondary' }}"
                        onclick="scopeTerm('-1')">오늘
                </button>
                <button type="button"
                        class="btn-pick btn btn-sm {{ date("Y-m-d",strtotime ("+7 days", strtotime(request()->input('start_date')))) == date('Y-m-d') ? 'btn-default':'btn-secondary' }}"
                        onclick="scopeTerm('-7')">1주일
                </button>
                <button type="button"
                        class="btn-pick btn btn-sm {{ date("Y-m-d",strtotime ("+30 days", strtotime(request()->input('start_date')))) == date('Y-m-d') ? 'btn-default':'btn-secondary' }}"
                        onclick="scopeTerm('-30')">1개월
                </button>
                <button type="button"
                        class="btn-pick btn btn-sm {{ date("Y-m-d",strtotime ("+90 days", strtotime(request()->input('start_date')))) == date('Y-m-d') ? 'btn-default':'btn-secondary' }}"
                        onclick="scopeTerm('-90')">3개월
                </button>
                <button type="button"
                        class="btn-pick btn btn-sm {{ date("Y-m-d",strtotime ("+180 days", strtotime(request()->input('start_date')))) == date('Y-m-d') ? 'btn-default':'btn-secondary' }}"
                        onclick="scopeTerm('-180')">6개월
                </button>
            </div>&nbsp;&nbsp;&nbsp;&nbsp;
            ...
            <div class="row item-info">
                ...
                <div class="col-xs-7">
                    ...
                    <p><b>주문일자</b> : {{ $order->created_at->format('Y-m-d') }}&nbsp;&nbsp;<b>가격</b>
                        : {{ number_format($order->amount) . "원"  }}</p>
                    <div class="btn-group" role="group" aria-label="status">
                        ...
                        @if($order->status == '구매결정')
                            <div class="btn-group">
                                <button type="button" class="btn btn-sm btn-default">리뷰작성</button>
                                ...
                                <ul class="dropdown-menu" role="menu">
                                    @foreach($order->items as $item)
                                        <li>
                                            <a href="{{ route('reviews.create', $item->option->product->id) }}">{{ $item->option->product->ad_title }}</a>
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
...
@section('script')
    ...
    <script>
        ...
        $(".btn__review").click(function () {
            var id = $(this).val();
            location.href = '/reviews/create/' + id;
        });
